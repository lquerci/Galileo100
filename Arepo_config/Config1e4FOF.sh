...
EVALPOTENTIAL                            # computes gravitational potential (NEEDED FOR OUTPOTENTIAL)
...
FOF_PRIMARY_LINK_TYPES=8      # 2^type for the primary dark matter type
FOF_SECONDARY_LINK_TYPES=1+2 # 2^type for the types linked to nearest primaries
...
FOF_GROUP_MIN_LEN=10000          # Minimum number of particles in one group (default is 32)
...
#OUTPUTTIMESTEP                           # output timestep of particle (RISTUDIDA)
OUTPUTPOTENTIAL                          # output potential at particle position (NEED EVALPOTENTIAL)
